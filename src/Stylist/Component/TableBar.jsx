...
const TableBar = () => {
  ...
  const generatePDF = () => {
    try {
      ...
      doc.setFontSize(18);
      doc.text('Laporan Inventaris', 14, 20);
      doc.setFontSize(12);
      ...
      Swal.fire({
        icon: 'error',
        title: 'Kesalahan',
        text: 'Gagal membuat laporan PDF'
      });
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      ...
      if (success) {
        ...
        Swal.fire('Kesalahan!', 'Gagal menghapus barang.', 'error');
      }
    }
  };
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div>
              <div className="flex items-center gap-3 mb-1">
                ...
                <h2 className="text-3xl font-extrabold text-slate-800 tracking-tight">Daftar Inventaris</h2>
              </div>
              <p className="text-sm text-slate-500 font-medium ml-4">Kelola dan pantau aset inventaris Anda dengan presisi</p>
            </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50/50 text-slate-500 sticky top-0 z-10 backdrop-blur-md">
              <tr className="border-b border-slate-100">
                ...
                <th className="px-6 py-5 text-[11px] font-bold uppercase tracking-[0.2em] text-center">Stok</th>
                <th className="px-6 py-5 text-[11px] font-bold uppercase tracking-[0.2em] text-center">Status</th>
                <th className="px-8 py-5 text-[11px] font-bold uppercase tracking-[0.2em] text-right">Aksi</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
              {displayedItems.length > 0 ? (
                displayedItems.map((item, index) => (
                  <tr key={item.idBarang} className="hover:bg-indigo-50/30 transition-all duration-300 group">
                    <td className="px-8 py-4">
                      <div className="w-16 h-16 rounded-2xl overflow-hidden bg-white shadow-sm ring-1 ring-slate-100 group-hover:ring-indigo-200 transition-all duration-500 transform group-hover:scale-105">
                        {item.publicUrl ? (
                          <img
                            src={item.publicUrl}
                            alt={item.namaBarang}
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <div className={`w-2 h-2 rounded-full ${item.kategori === 'Gear' ? 'bg-amber-400' : 'bg-indigo-400'}`} />
                    ...
                    <td className="px-8 py-4 text-right">
                      <div className="flex items-center justify-end gap-2 transition-all duration-300">
                        <Link to={`/home/EditBarang/${item.idBarang}`}>
                          <button className="p-2.5 bg-white text-indigo-600 hover:bg-indigo-600 hover:text-white rounded-xl shadow-sm border border-slate-100 transition-all duration-300 active:scale-90" title="Ubah">
                ...
                <tr>
                  <td colSpan="7" className="p-20 text-center text-slate-400">
                    <div className="flex flex-col items-center gap-4">
                      ...
                      <h3 className="text-lg font-bold text-slate-600">Aset tidak ditemukan</h3>
                      <p className="text-sm text-slate-400 max-w-xs mx-auto">Kami tidak dapat menemukan barang yang sesuai dengan kriteria pencarian Anda.</p>
                    </div>
                  </td>
                </tr>
              )}
